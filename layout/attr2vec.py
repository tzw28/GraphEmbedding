...
# import csrgraph
from layout.attr.csrgraph import CSRGraph
...
def init_attributed_graph(G, skip=False):
    ...
    for node in nodes:
        ...
        for attr_key in attr_dict.keys():
            virtual_node_name = (
                "_" + attr_key + "_" + str(attr_dict[attr_key]).replace(" ", "")
            )
...
class attr2vec(object):
    def __init__(
        ...
        self.G = G
        self.temp_G = G.copy()
        virtual_nodes = init_attributed_graph(self.temp_G)
        self.g2v = Attr2Vec(
            n_components=d,
            walklen=walklen,
            epochs=epochs,
    ...
    def train(self):
        self.g2v.fit(self.temp_G)
        self.g2v.save_vectors("./emb")

    def get_embeddings(self):
        
        model = KeyedVectors.load_word2vec_format("emb")
        nodes = self.G.nodes
        vectors = {}
        for node in nodes:
        ...
        return vectors
        vectors = {}
        for i, key in enumerate(self.g2v.model.wv.index2word):
            vectors[key] = self.g2v.model.wv.vectors[i]
        clean_attributed_graph(self.temp_G, vectors)
        return vectors
