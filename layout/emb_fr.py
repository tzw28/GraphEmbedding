...
def fast_kmeans(vectors, k):
    vector_list = []
    node_list = []
    for node in vectors.keys():
        node_list.append(node)
        vector_list.append(vectors[node])
    ds = np.array(vector_list)
    result, _ = kmeans_xufive(ds, k)
    result_dict = {}
    for i, node in enumerate(node_list):
        result_dict[node] = result[i]
    return result_dict
...
def embedding_fr(G, vectors, pos=None, te=0.6, wa=1, we=1,
                 ...
                 tel=0.6, teh=0.85):
    pos, t1, t2, t3 = attr_fruchterman_reingold_layout(
...
def __fast_sim_matrix(vec_list, size):
    ...
    for i in range(0, size):
        for j in range(0, size):
            sim_mat[i][j] = euclidean_distance_square_numba_v3(vec_list[i], vec_list[j])
...
def attr_fruchterman_reingold_layout(
    ...
    if vectors is not None:
        ...
        sim_end = time.time()
        sim_time = round(sim_end - sim_start, 3)
    ...
    try:
        ...
        pos_end = time.time()
        pos_time = round(pos_end - pos_start, 3)
    except:
        ...
        A = _embed_adjacency_matrix(A, E, wa, we, te,
                                    cluster_list=cluster_list, tel=tel, teh=teh)
        ea_end = time.time()
        ea_time = round(ea_end - ea_start, 3)
        if k is None and fixed is not None:
            # We must adjust k by domain size for layouts not near 1x1
            nnodes, _ = A.shape
            k = dom_size / np.sqrt(nnodes)
    ...
    pos = dict(zip(G, pos))
    return pos, sim_time, ea_time, pos_time
