...
def plot_embeddings(G, embeddings,):
    ...
    plt.savefig("./pics/miserables/{}-{}-{}/sv-{}-{}-{}-emb-real.png".format(
        t.year, t.month, t.day,
        t.hour, t.minute, int(t.second)
    ))
    plt.cla()
    cluster = kmeans(embeddings, K=4)
    cluster_color = []
    for node in G.nodes:
    ...
    plt.savefig("./pics/miserables/{}-{}-{}/sv-{}-{}-{}-emb-cluster.png".format(
        t.year, t.month, t.day,
        t.hour, t.minute, int(t.second)
    ))
    plt.cla()
...
def plot_layout(G, embeddings, color_dict, cluster):
    ...
    plt.savefig("./pics/miserables/{}-{}-{}/sv-{}-{}-{}-real.png".format(
        t.year, t.month, t.day,
        t.hour, t.minute, int(t.second)
    ))
    plt.cla()
    ...
    nx.draw_networkx_nodes(G, pos, node_size=80, node_color=cluster_color, edgecolors='white', linewidths=0.7)
    nx.draw_networkx_edges(G, pos, width=0.7, alpha=0.2)
    plt.savefig("./pics/miserables/{}-{}-{}/sv-{}-{}-{}-cluster.png".format(
        t.year, t.month, t.day,
        t.hour, t.minute, int(t.second)
    ))
...
if __name__ == "__main__":
    ...
    color_dict, cluster = plot_embeddings(G, embeddings)
