...
def plot_embeddings(embeddings,):
    ...
    plt.savefig("./pics/miserables/{}-{}-{}/nv-{}-{}-{}-emb-real.png".format(
    ...
    plt.savefig("./pics/miserables/{}-{}-{}/nv-{}-{}-{}-emb-cluster.png".format(
        t.year, t.month, t.day,
        t.hour, t.minute, int(t.second)
    ))
    plt.cla()
...
def plot_layout(G, embeddings, color_dict, cluster):
    ...
    plt.savefig("./pics/miserables/{}-{}-{}/nv-{}-{}-{}-real.png".format(
        t.year, t.month, t.day,
        t.hour, t.minute, int(t.second)
    ))
    plt.cla()
    ...
    nx.draw_networkx_nodes(G, pos, node_size=80, node_color=cluster_color, edgecolors='white', linewidths=0.7)
    nx.draw_networkx_edges(G, pos, width=0.7, alpha=0.2)
    plt.savefig("./pics/miserables/{}-{}-{}/nv-{}-{}-{}-cluster.png".format(
        t.year, t.month, t.day,
        t.hour, t.minute, int(t.second)
    ))
...
def generate_mis():
    label_dict = {}
    edges = []
    with open("E:\\学期文件\\毕设\\代码\\Visual-Analysis-Demo\\data\\miserables.json", "r") as f:
        text = f.read()
    ...
    for node in nodes:
        ...
        label_dict[id] = label
    for link in links:
        edges.append((link["source"].replace(" ", ""), link["target"].replace(" ", "")))
    with open("./data/miserables/miserables_category.txt", "w") as f:
        line_str = "{} {}\n"
        for node, label in label_dict.items():
            f.write(line_str.format(node, label))
    with open("./data/miserables/miserables_label.txt", "w") as f:
        line_str = "{} {}\n"
        for node, label in label_dict.items():
            f.write(line_str.format(node, label))
    with open("./data/miserables/miserables_edgelist.txt", "w") as f:
        line_str = "{} {}\n"
        for s, t in edges:
            f.write(line_str.format(s, t))
...
if __name__ == "__main__":
    ...
    model = Node2Vec(G, walk_length=10, num_walks=80,
                     p=0.25, q=4, workers=1, use_rejection_sampling=1)
